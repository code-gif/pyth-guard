...
Written, tested, and building on Aiken v1.1.21 / stdlib v3.0.0 / Plutus V3:
...
- **`lib/pyth_guard/tests.ak`** — covering every rejection reason, both
  validity-bound encodings, the microsecond freshness boundary, and the
...
- **`monitor/`** — store, Pyth Pro websocket client, chain poller, drift
  analysis, CLI, 59 tests. `python -m pythmon demo --check` runs the full
...
Measure execution units against realistic multi-feed payloads. A ~7 KB script
is unremarkable, but the upstream parser is not free and the cost should be
...
`pyth-lazer-cardano` requires v3.0.0. Adding `fuzz` therefore reintroduces the
exact stdlib conflict that makes the Aiken compiler exit non-zero with no
diagnostic. Unblocked by a `fuzz` release targeting stdlib v3; until then the
suite uses explicit boundary cases, which cover the same edges less
generatively.
...
- **The silent-failure trap.** A stdlib version mismatch produces no error
  message at all. Documented in the README and the troubleshooting table
  because anyone integrating will eventually hit it.
